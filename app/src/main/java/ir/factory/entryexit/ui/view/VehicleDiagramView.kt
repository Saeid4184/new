...
class VehicleDiagramView @JvmOverloads constructor(
    ...
    // ---- transform: base "fit inside" scale, then user pinch/pan on top of it ----
    ...
    // Bug fix: this used to fit the diagram to the view's WIDTH only ("width /
    // DIAGRAM_CANVAS_WIDTH"). Since the canvas is much taller than it is wide (900x460 ≈ 1.96:1,
    // portrait vehicle top-view), fitting to width alone made the drawing far taller than the
    // card ever gets — so only the very top sliver (e.g. سقفی/سپر جلو) was ever visible, cropped,
    // while the rest of the (correctly-sized) card underneath sat empty/unused. Fitting to
    // whichever of width/height is more constraining shows the whole diagram inside the card.
    private fun recomputeBaseScale() {
        if (width == 0 || height == 0) return
        val widthScale = width / InspectionCatalog.DIAGRAM_CANVAS_WIDTH
        val heightScale = height / InspectionCatalog.DIAGRAM_CANVAS_HEIGHT
    ...
    // When the whole diagram now fits inside the view on a given axis (the common case at
    // baseScale, before any pinch-zoom), center it on that axis instead of pinning it to the
    // top-left corner, so leftover space is split evenly rather than dumped below/beside it.
    private fun clampPan() {
        val scale = baseScale * userScale
        val contentW = InspectionCatalog.DIAGRAM_CANVAS_WIDTH * scale
        val contentH = InspectionCatalog.DIAGRAM_CANVAS_HEIGHT * scale
        panX = if (contentW <= width) -(width - contentW) / 2f else panX.coerceIn(0f, contentW - width)
        panY = if (contentH <= height) -(height - contentH) / 2f else panY.coerceIn(0f, contentH - height)
    }
