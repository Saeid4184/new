...
object Fleet {
    ...
    /** The factory's own categorized driver roster — رانندگان کمپرسی‌تریلی/پمپ/میکسر/لودر —
     *  given directly (not derived from [vehicles]), used to group the Driver tab into the same
     *  kind of sections the Machinery tab already has. [group] doubles as the section header,
     *  same as [FleetVehicle.groupLabel] above. See
     *  [ir.factory.entryexit.data.Repository.ensureDriverCategoriesSeeded]. */
    private val driverCategories: List<Pair<String, List<String>>> = listOf(
        "رانندگان کمپرسی تریلی" to listOf(
            "سیروس انصاری", "رضا شمس", "مهدی حبیبی", "نصرت الله حیدری", "جمشید غلامی",
            "علی کریمی", "مجید مولایی", "محمد ایلخانی", "میثم بیگی"
        ),
        "رانندگان پمپ" to listOf(
            "سعید نوروزی", "عباس اسدی", "نایب سلیمانی", "جواد محمدی نیا", "رضا عسگری",
            "علی قوی بدن"
        ),
        "رانندگان میکسر" to listOf(
            "علی اصغر دیندار", "ایرج رزمجو", "سلیم مقدوری", "محمد رثایی", "آرمان منصف",
            "حمیدرضا عبدی", "مرتضی دباغی", "پیمان اعتصامی", "حشمت اسدی", "حمید سوری",
            "رضا پروهان", "رضا جعفری", "محسن آزاد", "برهان تغتمش", "قاسم بخشی",
            "سردار شکوری", "محسن قنبری", "حسین شهبازی", "هرمز محمودیان", "محمد قنبری",
            "محمود علیپور", "کامبیز گوزلی", "عباس عزیزی فرد", "داود لطفی", "بخشعلی شمس",
            "عباس صالحی", "حسین ناظری", "ابراهیم زرگر"
        ),
        "رانندگان لودر" to listOf(
            "حسین رازگردانی", "محمد براتی"
        )
    )

    /** Flattened (name, group) pairs for [driverCategories], used by the seeding migration. */
    fun categorizedDrivers(): List<Pair<String, String>> =
        driverCategories.flatMap { (group, names) -> names.map { name -> name to group } }
}
