...
interface PersonDao {
    ...
    /** Exact name match within a type — used to check whether a roster entry already exists
     *  before creating a duplicate (e.g. merging in the categorized driver list). */
    @Query("SELECT * FROM persons WHERE type = :type AND name = :name LIMIT 1")
    suspend fun getByNameAndType(type: String, name: String): PersonEntity?
